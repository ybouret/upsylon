...
Y_UTEST(fitting_circles)
{
    ...
    for(size_t i=3+alea.leq(30);i>0;--i)
    {
        ...
        const double x   = xc + r*cos(phi)*(1.0f+0.1f*alea.symm<double>());
        const double y   = yc + r*sin(phi)*(1.0f+0.1f*alea.symm<double>());

        i_circle.add( unit_t(x), unit_t(y) );
        d_circle.add( unit_t(x), unit_t(y) );
    }
