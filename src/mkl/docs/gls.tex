...
We should have:
\begin{equation}
	f(\gamma) \approx f_0 - 2\gamma \sigma + \gamma^2 \left(f_1-f_0\right)
\end{equation}

\begin{itemize}
\item If $f_1>f_0$, we need to increase $\lambda$ and recompute the step from Eq.\eqref{start}.
\item if $f_1\leq f_0$, then accept step an decrease $\lambda$.
\end{itemize}
