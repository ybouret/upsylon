...
\begin{equation}
	\forall s \in [1:S], \; F(\vec{X},\vec{A}_s), \;\; \vec{A}_s = \bm{\Gamma}_s \vec{A}, \;\; \bm{\Gamma}_s \in \mathbb{M}_{m,M},\;\;m\leq M
\end{equation}

The least square a sample is:
\begin{equation}
...
\begin{array}{rcl}
	\vec{\beta}  & = & - \vec{\nabla}_{\vec{A}} D^2 \\
	& = & - \dfrac{1}{N} \sum_s N_s \trn{\bm{\Gamma}}_s \vec{\nabla}_{\vec{A}_s} D^2_s\\
	& = & \dfrac{1}{N} \left[\sum_s N_s \trn{\bm{\Gamma}}_s \left( \sum_{i=1}^{N_s} \left[ Y_{si} - F(\vec{X}_{si},\vec{A}_s) \right] \vec{\nabla} F(\vec{X}_{si},\vec{A}_s)\right)\right]\\
\end{array}
\end{equation}
