...
We define a balancing function $B(C)$ with indicates if $C$ is valid or not.
Then we define the global balance:
\begin{equation}
	\mathcal{B}(\vec{C}) = \sum_j \mathbb{A}_j B(C_j)
\end{equation}
where $\mathbb{A}_j$ is $1$ is species $j$ is active, $0$ otherwise.
...
\begin{equation}
	...
	\end{pmatrix}
\end{equation}
which may have between $0$ and $M_A$ not-zero coefficients, 
and
\begin{equation}
	\mathcal{B}(\vec{C}+\trn{\bm{\nu}}\vec{\xi}) \simeq 
	\mathcal{B}(\vec{C}) + \bm{\nu} \vec{\nabla}\mathcal{B} \cdot \vec{\xi}
\end{equation}
So the balancing directions are in the form:
\begin{equation}
	\vec{\xi} = - \bm{\Omega} \left(\bm{\nu} \vec{\nabla}\mathcal{B}\right), \;\; \bm{\Omega} \in \mathcal{M}_{N}, \;\; \bm{\Omega}\geq 0
\end{equation}
leading to 
\begin{equation}
	\delta\vec{C} = \trn{\bm{\nu}} \vec{\xi}  = -\trn{\bm{\nu}} \bm{\Omega} \nu \vec{\nabla}\mathcal{B}
...
And $\bm{\Omega}$ is chosen so that:
\begin{equation}
\forall \mathbb{A}_j \not=0, \;
\left\lbrace
	\begin{array}{rcl}
	C_j < 0      & \Rightarrow & \delta {C_j} \geq 0 \\
	C_j \geq 0   & \Rightarrow & \delta {C_j} \geq -C_j \\
	\end{array}
\right.
\end{equation}
and obviously $\mathbb{A}_j = 0 \Rightarrow \delta {C_j}=0$.\\
We need to find a set of $N$ valid extents to satisfy those $M_A$ balancing
conditions. Due to the mass conservation......\\

\begin{equation}
	\delta \vec{C} \geq \vec{X}
\end{equation}
