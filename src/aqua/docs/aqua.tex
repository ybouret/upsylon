...
\begin{equation}
	\vec{C}=\vec{C}_0 + \trn{\bm{\nu}} \vec{\xi}
\end{equation}
so that the search direction is:
\begin{equation}
	-\partial_{\vec{\xi}} E = \bm{\nu}
	\begin{bmatrix}
		\vdots\\
		\max(0,-C_j)\\
		\vdots\\
	\end{bmatrix}
\end{equation}
and for each step the {\it minimal} length is computed.
