...
#define YJS_PRINTLN(CODE) do { if(Scanner::Verbose) { std::cerr << "[" << label << "] " << CODE << std::endl; } } while(false)
            ...
            Unit * Scanner:: endOfStream(const Source &source) const
            {
                ...
                switch(atEOS)
                {
                    case RejectEOS: {
                        ...
                        //------------------------------------------------------
                        exception excp;
                        source.context().stampTo(excp).cat("[%s] unexpected end of stream",**label);
                        throw excp;
                    }
            ...
            PROBE:
                ...
                {
                    exception excp;
                    source.context().stampTo(excp).cat("[%s] unexpected failure for <%s> to accept '%s...'!",**label,**(bestRule->label),cchars::visible[code]);
                }
            ...
            exception Scanner:: syntaxError(Source &source) const
            {
                ...
                exception excp;
                {
                    ...
                    //----------------------------------------------------------
                    token.push_back( source.get() )->stampTo(excp);
