...
#define TAB_HEIGHT 21	// fixed height of tab button icons
        ...
        //
        int Gel_Tabs::tab_positions(int* p, int* wp, int &tw) {
            int selected = 0;
        ...
        void Gel_Tabs::tab_area(int& H, int& yedge) {
            H = h();
            int H2 = y();
            Fl_Widget*const* a = array();
        ...
        Fl_Widget *Gel_Tabs::which(int event_x, int event_y) {
        ...
        int Gel_Tabs::value(Fl_Widget *newvalue) {
            Fl_Widget*const* a = array();
            int ret = 0;
            for (int i=children(); i--;) {
                Fl_Widget* o = *a++;
