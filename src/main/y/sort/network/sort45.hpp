//! \file
#ifndef Y_NW_sort45_INCLUDED
#define Y_NW_sort45_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 45 items
	template <> struct network_sort<45> {
		static const unsigned I[413]; //!< I swap indices
		static const unsigned J[413]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<413;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
