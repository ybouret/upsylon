//! \file
#ifndef Y_NW_sort33_INCLUDED
#define Y_NW_sort33_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 33 items
	template <> struct network_sort<33> {
		static const unsigned I[243]; //!< I swap indices
		static const unsigned J[243]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<243;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
