//! \file
#ifndef Y_NW_sort52_INCLUDED
#define Y_NW_sort52_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 52 items
	template <> struct network_sort<52> {
		static const unsigned I[515]; //!< I swap indices
		static const unsigned J[515]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<515;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
