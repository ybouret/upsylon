//! \file
#ifndef Y_NW_sort37_INCLUDED
#define Y_NW_sort37_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 37 items
	template <> struct network_sort<37> {
		static const unsigned I[308]; //!< I swap indices
		static const unsigned J[308]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<308;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
