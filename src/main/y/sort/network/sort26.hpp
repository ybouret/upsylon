//! \file
#ifndef Y_NW_sort26_INCLUDED
#define Y_NW_sort26_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 26 items
	template <> struct network_sort<26> {
		static const unsigned I[163]; //!< I swap indices
		static const unsigned J[163]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<163;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
