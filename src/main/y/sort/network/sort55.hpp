//! \file
#ifndef Y_NW_sort55_INCLUDED
#define Y_NW_sort55_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 55 items
	template <> struct network_sort<55> {
		static const unsigned I[558]; //!< I swap indices
		static const unsigned J[558]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<558;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
