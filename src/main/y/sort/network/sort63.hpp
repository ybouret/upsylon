//! \file
#ifndef Y_NW_sort63_INCLUDED
#define Y_NW_sort63_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 63 items
	template <> struct network_sort<63> {
		static const unsigned I[659]; //!< I swap indices
		static const unsigned J[659]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<659;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
