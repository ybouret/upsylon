//! \file
#ifndef Y_NW_sort49_INCLUDED
#define Y_NW_sort49_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 49 items
	template <> struct network_sort<49> {
		static const unsigned I[470]; //!< I swap indices
		static const unsigned J[470]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<470;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
