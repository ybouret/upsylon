//! \file
#ifndef Y_NW_sort39_INCLUDED
#define Y_NW_sort39_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 39 items
	template <> struct network_sort<39> {
		static const unsigned I[331]; //!< I swap indices
		static const unsigned J[331]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<331;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
