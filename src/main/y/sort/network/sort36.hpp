//! \file
#ifndef Y_NW_sort36_INCLUDED
#define Y_NW_sort36_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 36 items
	template <> struct network_sort<36> {
		static const unsigned I[288]; //!< I swap indices
		static const unsigned J[288]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<288;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
