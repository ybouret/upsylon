//! \file
#ifndef Y_NW_sort48_INCLUDED
#define Y_NW_sort48_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 48 items
	template <> struct network_sort<48> {
		static const unsigned I[438]; //!< I swap indices
		static const unsigned J[438]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<438;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
