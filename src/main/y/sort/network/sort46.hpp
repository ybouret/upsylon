//! \file
#ifndef Y_NW_sort46_INCLUDED
#define Y_NW_sort46_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 46 items
	template <> struct network_sort<46> {
		static const unsigned I[422]; //!< I swap indices
		static const unsigned J[422]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<422;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
