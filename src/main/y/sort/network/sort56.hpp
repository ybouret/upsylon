//! \file
#ifndef Y_NW_sort56_INCLUDED
#define Y_NW_sort56_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 56 items
	template <> struct network_sort<56> {
		static const unsigned I[565]; //!< I swap indices
		static const unsigned J[565]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<565;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
