//! \file
#ifndef Y_NW_sort51_INCLUDED
#define Y_NW_sort51_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 51 items
	template <> struct network_sort<51> {
		static const unsigned I[505]; //!< I swap indices
		static const unsigned J[505]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<505;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
