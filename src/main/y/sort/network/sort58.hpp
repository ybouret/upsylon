//! \file
#ifndef Y_NW_sort58_INCLUDED
#define Y_NW_sort58_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 58 items
	template <> struct network_sort<58> {
		static const unsigned I[602]; //!< I swap indices
		static const unsigned J[602]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<602;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
