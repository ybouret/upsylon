//! \file
#ifndef Y_NW_sort43_INCLUDED
#define Y_NW_sort43_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 43 items
	template <> struct network_sort<43> {
		static const unsigned I[389]; //!< I swap indices
		static const unsigned J[389]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<389;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
