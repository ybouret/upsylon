//! \file
#ifndef Y_NW_sort44_INCLUDED
#define Y_NW_sort44_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 44 items
	template <> struct network_sort<44> {
		static const unsigned I[397]; //!< I swap indices
		static const unsigned J[397]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<397;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
