//! \file
#ifndef Y_NW_sort54_INCLUDED
#define Y_NW_sort54_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 54 items
	template <> struct network_sort<54> {
		static const unsigned I[546]; //!< I swap indices
		static const unsigned J[546]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<546;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
