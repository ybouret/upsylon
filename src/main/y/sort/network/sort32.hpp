//! \file
#ifndef Y_NW_sort32_INCLUDED
#define Y_NW_sort32_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 32 items
	template <> struct network_sort<32> {
		static const unsigned I[211]; //!< I swap indices
		static const unsigned J[211]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<211;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
