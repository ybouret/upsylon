//! \file
#ifndef Y_NW_sort41_INCLUDED
#define Y_NW_sort41_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 41 items
	template <> struct network_sort<41> {
		static const unsigned I[362]; //!< I swap indices
		static const unsigned J[362]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<362;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
