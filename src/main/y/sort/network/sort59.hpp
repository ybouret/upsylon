//! \file
#ifndef Y_NW_sort59_INCLUDED
#define Y_NW_sort59_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 59 items
	template <> struct network_sort<59> {
		static const unsigned I[616]; //!< I swap indices
		static const unsigned J[616]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<616;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
