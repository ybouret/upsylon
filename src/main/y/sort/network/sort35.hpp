//! \file
#ifndef Y_NW_sort35_INCLUDED
#define Y_NW_sort35_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 35 items
	template <> struct network_sort<35> {
		static const unsigned I[278]; //!< I swap indices
		static const unsigned J[278]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<278;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
