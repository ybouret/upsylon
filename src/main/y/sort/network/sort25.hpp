//! \file
#ifndef Y_NW_sort25_INCLUDED
#define Y_NW_sort25_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 25 items
	template <> struct network_sort<25> {
		static const unsigned I[154]; //!< I swap indices
		static const unsigned J[154]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<154;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
