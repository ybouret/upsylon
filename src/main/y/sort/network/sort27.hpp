//! \file
#ifndef Y_NW_sort27_INCLUDED
#define Y_NW_sort27_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 27 items
	template <> struct network_sort<27> {
		static const unsigned I[173]; //!< I swap indices
		static const unsigned J[173]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<173;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
