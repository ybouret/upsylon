//! \file
#ifndef Y_NW_sort64_INCLUDED
#define Y_NW_sort64_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 64 items
	template <> struct network_sort<64> {
		static const unsigned I[665]; //!< I swap indices
		static const unsigned J[665]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<665;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
