//! \file
#ifndef Y_NW_sort47_INCLUDED
#define Y_NW_sort47_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 47 items
	template <> struct network_sort<47> {
		static const unsigned I[432]; //!< I swap indices
		static const unsigned J[432]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<432;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
