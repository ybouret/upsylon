//! \file
#ifndef Y_NW_sort40_INCLUDED
#define Y_NW_sort40_INCLUDED
#include "../network.hpp"
namespace upsylon {
		//! network sort for 40 items
	template <> struct network_sort<40> {
		static const unsigned I[338]; //!< I swap indices
		static const unsigned J[338]; //!< J swap indices
		//! sort...
		template <typename T,typename COMPARE> static inline
		void on(T *a, COMPARE &compare) throw() {
			assert(NULL!=a);
			for(unsigned k=0;k<338;++k){
				T &aI=a[I[k]], &aJ=a[J[k]];
