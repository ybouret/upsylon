...
#define               MATRIX_A      0x9908b0dfUL  /* constant vector a */
        ...
        void mersenne_twister:: initialize(const unsigned long u) throw()
        {
            mt[0]= u & 0xffffffffUL;
            for(mti=1; mti<N; mti++)
            {
        ...
        uint32_t mersenne_twister:: next32() throw()
        {
            return genrand_uint32(mti,mt);
        }

        void mersenne_twister:: reseed(bits &b) throw()
        {
            initialize( b.full<unsigned long>() );
        }
