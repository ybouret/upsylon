...
namespace upsylon {
    ...
    namespace Information {
        ...
        const uint8_t BitFormat::Table<uint8_t>::Bit[8] =
        {
            1,2,4,8,16,32,64,128
        };

        const uint16_t BitFormat::Table<uint16_t>::Bit[16] =
        {
            1,   2,   4,    8,    16,   32,   64,    128,
            256, 512, 1024, 2048, 4096, 8192, 16384, 32768
        };

        const uint32_t BitFormat::Table<uint32_t>::Bit[32] =
        {
        ...
        const uint64_t BitFormat::Table<uint64_t>::Bit[64] =
        {
...
namespace upsylon {
    ...
    namespace Information {
        ...
        void IOBits:: store( char C )
        {
            ...
            for(int i=7;i>=0;--i)
            {
                push_front(   0 != (U& BitFormat::Table<uint8_t>::Bit[i]) );
            }
        }
