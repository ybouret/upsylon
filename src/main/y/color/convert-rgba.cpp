...
namespace upsylon
{
    ...
    template <>
    uint8_t convert_color<uint8_t,rgba>:: from(const rgba &c) throw()
    {
        return core::convert_color::greyscale[ unsigned(c.r) + unsigned(c.g) + unsigned(c.b) ];
    }

    template <>
    float convert_color<float,rgba>:: from(const rgba &c) throw()
    {
    ...
    template <>
    rgb convert_color<rgb,rgba>:: from(const rgba &c) throw()
    {
    ...
    template <>
    YUV convert_color<YUV,rgba>:: from(const rgba &c) throw()
    {
