...
namespace upsylon {
    ...
    private:
        ...
        inline void put_to_sleep(node_type *node) throw() { collapse(dormant.store(node)->data); }
        ...
        {
            while( content.size ) { node_type *node = content.pop_back(); collapse(node->data); object::release1(node); }
