...
#include "y/crypto/bc/block-cipher-ecb.hpp"
#include "y/crypto/bc/block-cipher-cbc.hpp"
#include "y/crypto/bc/block-cipher-ctr.hpp"
#include "y/crypto/bc/block-cipher-cfb.hpp"
#include "y/crypto/bc/block-cipher-ofb.hpp"
    ...
    namespace crypto
    {
        ...
        void block_cipher_factory:: declare( const string &id, creator proc)
        {
            if( !db.insert(id,proc) )
                throw exception("block_cipher_library(multiple '%s')", *id);
        }
...
#define Y_BCF(ID) declare(#ID,ID)
        ...
        operating_block_cipher * block_cipher_factory:: create( const string &id, block_cipher &e, block_cipher &d, const memory::ro_buffer &iv) const
        {
            ...
            if(!hook)
                throw exception("%s.create( no '%s')", name, *id);
