...
/** static loop from SHIFT to (N-1+SHIFT), N>0 */
#define Y_LOOP_FUNC_STATIC_(N,FUNC,SHIFT) do                           \
{                                                                       \
static const size_t iter_      = (N);                                    \
static const size_t loop_init_ = (iter_+0x7)>>0x3;                        \
static const size_t jump_      = (iter_&0x7);                              \
static const size_t indx_init_ = jump_-(0x8-(SHIFT));                       \
assert(iter_>0);                                                             \
{                                                                             \
size_t       loop_ = loop_init_;                                               \
size_t       indx_ = indx_init_;                                                \
switch (jump_) {                                                                 \
case 0: do { { const size_t i0_ = (indx_ +=0x8); { FUNC(i0_); } }  /* FALLTHRU */ \
case 7:      { const size_t i1_ = indx_+0x1;     { FUNC(i1_); } }  /* FALLTHRU */ \
case 6:      { const size_t i2_ = indx_+0x2;     { FUNC(i2_); } }  /* FALLTHRU */ \
