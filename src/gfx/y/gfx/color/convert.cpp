...
namespace upsylon
{
    namespace GFX
    {
        uint8_t Conv:: Alpha(const uint8_t x, const uint8_t alpha) throw()
        {
            switch(alpha)
            {
                case 0:   return 0;
            ...
            return (unsigned(x) * unsigned(alpha))>>8;
        }
        
        
        //! conversion of bytes to triplet helper: 0->255*3
        const uint8_t Conv::Table[3*255+1] =
        {
